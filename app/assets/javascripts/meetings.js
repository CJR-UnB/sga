var populateTable = function(data) {
  ...
  if (data.length == 0) {
    ...
    var alert = '<div class="alert alert-danger"><i class="fa fa-exclamation-circle"></i>&nbsp;&nbsp;Não há reuniões para o núcleo selecionado.</div>';
    ...
    var table = $('<table id="meetings-table"></table>').addClass('table');
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      var link = "<td align='right'><a href=/meetings/" + data[i].id + ">Visualizar Reunião</a></td>";
...
var selectSectorAndPopulateTable = function(sector_id) {
  $.ajax('/sectors/' + sector_id + '/meetings.json', {
